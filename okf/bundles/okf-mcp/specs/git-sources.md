...
`read_git_source` resolves only declared source entries. It reads a regular blob from the pinned commit, verifies its Git object ID and SHA-256, enforces UTF-8 and byte limits, and rejects traversal, symbolic links, and submodules. It disables lazy fetches and never consults dirty working-tree bytes.
